...
TOO_FEW_SONG_ERROR = 'エラー：ソートする曲が不足しています。別のフィルターで再試行してください。';
...
getSortCompletedMessage = (date, timeTaken) => `終了時間${date}、所要時間：${msToReadableTime(timeTaken)}`;

DO_ANOTHER_SORTER = 'もう一度ソートする？';

IN_PROGRESS_TEXT = 'この後、「プログレスから読み込む」をクリックすると、このURLで再開または使用することができます。';
FINISHED_TEXT = 'このURLで結果を共有することもできますし、「最終結果を読み込む」をクリックして再度表示することもできます。';
